...
function populateAircraftInfoWindow(baseProps, plan, sortedRoutePoints, communityAircraftData) {
    ...
    const departureIcao = hasPlan ?
        originalFlatWaypointObjects[0]?.identifier ||
        originalFlatWaypointObjects[0]?.name : 'N/A';
    ...
    const depCountryCode = (typeof airportsData !== 'undefined' && airportsData[departureIcao]?.country) ?
        airportsData[departureIcao].country.toLowerCase() : '';
    ...
    const safeGetDistance = (typeof getDistanceKm === 'function') ?
        getDistanceKm : (lat1, lon1, lat2, lon2) => 0;
    ...
    if (originalFlatWaypointObjects.length > 0) {
        originalFlatWaypointObjects.forEach((wp, index) => {
            ...
            fmsLegsHtml += `
            <div class="fms-row ${index === 0 ? 'active-leg' : ''}" style="display: flex; justify-content: space-between;
    ...
    if (flightPhase.includes('CRUISE')) statusColor = '#38bdf8';
    // Blue
    if (flightPhase.includes('DESCENT')) statusColor = '#fbbf24';
    // Orange
    ...
    // --- HTML Construction ---
    // [FIX] Added explicit style rules to ac-header-modern (flex-shrink: 0, min-height, position relative)
    // to prevent it from collapsing when flex children expand.
    windowEl.innerHTML = `
    <div class="ac-header-modern" id="ac-overview-panel" style="background-image: url('${techCardImagePath}'), url('/CommunityPlanes/default.png'); 
        ...
        <div class="ac-header-overlay" style="position: absolute; inset: 0; background: rgba(0,0,0,0.4); z-index: 0;"></div>
        
        <div class="ac-header-top" style="position: relative; z-index: 1;">
        ...
        <div class="overview-actions" style="top: 20px; right: 20px; position: absolute; z-index: 2;">
        ...
        <div class="ac-header-route-strip" style="position: relative; z-index: 1;">
            ...
            <div class="route-visual">
                ...
                <div style="display: flex;
                    justify-content: space-between; width: 100%; margin-top: 6px; font-size: 0.65rem; color: #64748b;
                    font-weight: 700;">
        ...
        <div class="ac-info-window-tabs" style="padding: 12px 16px;
            display: flex; align-items: center; justify-content: space-between; gap: 16px; flex-shrink: 0;">
            <div class="modern-view-switcher" id="main-data-switcher" style="flex: 1;
                background: rgba(15, 23, 42, 0.4); border-radius: 12px; padding: 4px; display: flex; position: relative; border: 1px solid rgba(255,255,255,0.05);
                height: 44px;">
                 <button class="ac-info-tab-btn ${flightDataActiveClass}" data-tab="ac-tab-flight-data" style="flex: 1;
                border: none; background: transparent; color: #fff; font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 1.2px; padding: 0 10px; cursor: pointer;
                ...
                border: none; background: transparent; color: #94a3b8; font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 1.2px; padding: 0 10px; cursor: pointer;
                z-index: 1; transition: color 0.3s ease; display: flex; align-items: center; justify-content: center;
                gap: 8px;">
            ...
            <img src="Images/inflight.png" alt="Inflight Logo" class="ac-info-tab-logo" style="height: 24px;
      ...
      <div class="unified-display-main-content">
            <div id="ac-tab-flight-data" class="ac-tab-pane ${flightDataActiveClass}" style="gap: 6px;
                                ...
                                <svg width="787" height="800" viewBox="0 0 787 800" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <g id="PFD" clip-path="url(#clip0_1_2890)">
                                        <g transform="translate(0, 100)">
                                            ...
                                            <rect id="Rectangle 11" 
                                            x="527" y="532.777" width="168.696" height="262.947" transform="rotate(-51.9135 527 532.777)" fill="#030309"/>
                                            ...
                                            <path id="Polygon 2" d="M407.919 38.9482L396.431 59.4193L384.446 
                                            38.7244L407.919 38.9482Z" fill="#030309"/>
                                            <path id="Vector 6" d="M307 76L302 64.5L312 60.5L317 71" stroke="#E7E6E8" stroke-width="4"/>
                                            <path 
                                            id="Vector 7" d="M279.5 91L268.5 73.5L259 79L269.5 97.5" stroke="#E7E6E8" stroke-width="4"/>
                                            ...
                                            <path id="Ellipse 1" d="M636 481C636 484.866 632.866 488 629 488C625.134 488 622 484.866 622 481C622 477.134 625.134 474 629 474C632.866 474 636 
                                            477.134 636 481Z" fill="#D9D9D9"/>
                                            ...
                                            <g id="speed_indicator_static">
                                                ...
                                                <rect x="50" y="296" 
                                                width="45" height="30" fill="black" stroke="#999" stroke-width="1"/>
                                            ...
                                            <path id="Vector 21" 
                                            d="M29 73H151.5" stroke="#E7E6E8" stroke-width="4"/>
                                            <path id="Vector 22" d="M28 549H151.5" stroke="#E7E6E8" stroke-width="4"/>
                                            <path id="Vector 23" 
                                            d="M672.5 73H774" stroke="#E7E6E8" stroke-width="4"/>
                                            <path id="Vector 24" d="M672 548.5H773" stroke="#E7E6E8" stroke-width="4"/>
                                            <path id="Vector 25" 
                                            d="M745 549.5L746 347" stroke="#E7E6E8" stroke-width="3"/>
             ...
             <div class="info-right-col" style="gap: 6px;
    ...
    <div class="modern-status-card" style="background: linear-gradient(165deg, rgba(255,255,255,0.08) 0%, rgba(255,255,255,0.01) 100%);
        ...
        backdrop-filter: blur(12px); flex: 1; display: flex;
        flex-direction: column; justify-content: center;">
        <div class="status-glow" style="position: absolute; top: -20px; right: -20px;
        width: 60px; height: 60px; background: ${psColor}; filter: blur(35px); opacity: 0.2;"></div>
    ...
    <div class="modern-timer-stack" style="display: flex;
        ...
        color: #38bdf8; font-weight: 700;">--:--</span>
              ...
              <div class="tech-card" style="background: rgba(15, 23, 42, 0.6); border: 1px solid rgba(255,255,255,0.1); border-radius: 16px; overflow: hidden;
        ...
        color: #fff; font-weight: 500;">${photographerName}</span>
            ...
            <a href="#" style="width: 28px; height: 28px; background: rgba(56, 189, 248, 0.2); border-radius: 6px; color: #38bdf8; display: flex;
        align-items: center; justify-content: center; border: 1px solid rgba(56, 189, 248, 0.2);">
        ...
        font-size: 9px; color: #94a3b8; font-weight: 600;">CLASS-1</span>
          ...
          <div class="vsd-legend-item"><div class="dot-flown"></div> FLOWN</div>
            ...
            <div id="ac-tab-pilot-report" class="ac-tab-pane ${pilotReportActiveClass}" style="display: ${pilotReportDisplay};
        padding: 12px;">
