...
// Keep a localized cache of pilots tracked this session
// This prevents spamming the server if the user clicks the same plane multiple times.
const trackedSessionPilots = new Set();

function trackPilotView(flight) {
    // 1. Safety Check: Ensure valid data
    if (!flight || !flight.userId || !flight.username) return;

    // 2. Cache Check: If we already tracked them this session, stop here.
    // This saves bandwidth and reduces server load significantly.
    if (trackedSessionPilots.has(flight.userId)) {
        return;
    }

    // 3. Mark as tracked immediately (optimistic update)
    trackedSessionPilots.add(flight.userId);

    const BACKEND = "https://site--indgo-backend--6dmjph8ltlhv.code.run";

    // 4. Fire-and-Forget Request
    // We do NOT use 'await' here. We want the UI to continue immediately 
    // without waiting for the server to say "OK".
    fetch(`${BACKEND}/api/leaderboard/track`, {
        method: 'POST',
        ...
        // If it fails, we remove them from the cache so we can try again later
        console.warn("❌ Leaderboard tracking failed, retrying next time:", e);
        trackedSessionPilots.delete(flight.userId);
    });
}
