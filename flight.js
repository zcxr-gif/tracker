...
function setupFilterSettingsWindowEvents() {
   ...
   /**
    ...
    /**
     * --- [NEW] Centralized Message Listener for Iframes ---
     * Handles communication from both the simple flight window (actions)
     * and the Nav Display (ND_READY handshake).
     */
    function setupGlobalMessageListener() {
        window.addEventListener('message', (event) => {
            ...
            // 1. Handshake from Nav Display
            if (data.type === 'ND_READY') {
                refreshNavDisplayFromCache();
                return;
            }

            // 2. "Close" Action from Simple Flight Window
            if (data.type === 'CLOSE_SIMPLE_WINDOW') {
                if (aircraftInfoWindow) {
                    aircraftInfoWindow.classList.remove('visible');
                    if (window.MobileUIHandler) window.MobileUIHandler.closeActiveWindow();
                    ...
                    // Cleanup logic (mirrors setupAircraftWindowEvents)
                    if (currentFlightInWindow) {
                        clearLiveFlightPath(currentFlightInWindow);
                        liveTrailCache.delete(currentFlightInWindow);
                    }
            ...
            // 3. "Hide" Action from Simple Flight Window
            if (data.type === 'HIDE_SIMPLE_WINDOW') {
                if (aircraftInfoWindow) {
                    aircraftInfoWindow.classList.remove('visible');
                    if (currentFlightInWindow) {
                        clearLiveFlightPath(currentFlightInWindow);
                    ...
                    // Show Recall Button with Animation
                    if (aircraftInfoWindowRecallBtn) {
                        aircraftInfoWindowRecallBtn.classList.add('visible', 'palpitate');
                        setTimeout(() => aircraftInfoWindowRecallBtn.classList.remove('palpitate'), 1000);
                    }
            ...
            // 4. "Open Profile" from Simple Flight Window
            if (data.type === 'OPEN_PROFILE') {
                console.log("Open Profile requested from Simple Window");
            }
        });
    }
