...
/**
    ...
    async function initializeSectorOpsView() {
        ...
        try {
            ...
            // --- 5. Inject Filter Settings Window (UPDATED WITH LAYOUT CONTROLS) ---
                ...
                const windowHtml = `
                    <div id="filter-settings-window" class="info-window">
                        ...
                        <div id="filter-window-content" class="info-window-content">
                            ...
                            <div class="filter-section-divider">
                                <span class="filter-section-title">Window Layout</span>
                            </div>
                            <ul class="filter-toggle-list" id="layout-mode-group" style="padding-top: 8px;">
                                <li class="filter-radio-item">
                                    <input type="radio" id="layout-mode-default" name="window-layout-mode" value="default" checked>
                                    <label for="layout-mode-default"><i class="fa-solid fa-columns"></i> Standard (PFD Left)</label>
                                </li>
                                <li class="filter-radio-item">
                                    <input type="radio" id="layout-mode-sidebyside" name="window-layout-mode" value="sidebyside">
                                    <label for="layout-mode-sidebyside"><i class="fa-solid fa-table-columns"></i> Side-by-Side (Equal)</label>
                                </li>
                                <li class="filter-radio-item">
                                    <input type="radio" id="layout-mode-compact" name="window-layout-mode" value="compact">
                                    <label for="layout-mode-compact"><i class="fa-solid fa-bars"></i> Vertical Stack</label>
                                </li>
                                <li class="filter-radio-item">
                                    <input type="radio" id="layout-mode-inverted" name="window-layout-mode" value="inverted">
                                    <label for="layout-mode-inverted"><i class="fa-solid fa-arrow-right-arrow-left"></i> Inverted (PFD Right)</label>
                                </li>
                            </ul>
...
/**
 * --- [NEW] Applies the selected CSS Grid layout to the aircraft info window.
 * This allows the user to switch between Default, Side-by-Side, Compact, etc.
 * @param {string} mode - The layout mode identifier ('default', 'sidebyside', 'inverted', 'compact').
 */
function applyCustomLayout(mode) {
    const layoutContainer = document.getElementById('user-custom-layout');
    
    // Safety check: If the window isn't open, this element won't exist.
    ...
    switch (mode) {
        case 'sidebyside':
            // 50/50 split between PFD and the Sidebar
            // Nav and VSD sit below them.
            styleString = `
        ...
        case 'inverted':
            // PFD moves to the RIGHT column.
            // Sidebar moves to the LEFT column.
            styleString = `
        ...
        case 'compact':
            ...
            // Useful for smaller screens or simple views.
            styleString = `
                grid-template-columns: 1fr;
                grid-template-areas: 
                    "pfd"
        ...
        case 'default':
        default:
            // The standard layout: PFD takes 2/3rds width on the left.
            styleString = `
                grid-template-columns: 2fr 1fr;
                grid-template-areas: 
                    "pfd    side"
    ...
    // Apply the constructed grid string directly to the container's style
    layoutContainer.style.cssText = styleString;
    
    // Persist the user's choice to local storage so it remembers next time
    localStorage.setItem('userLayoutMode', mode);
    
    // Log for debugging
    console.log(`Layout applied: ${mode}`);
}


/**
 * --- [REHAULED] Populates the aircraft info window with the new Tech Card UI.
 * This version introduces a CSS Grid layout system for user customization.
 */
function populateAircraftInfoWindow(baseProps, plan, sortedRoutePoints) {
    ...
    <style>
        ...
        /* ... (Include previous Tech Card & FMS Styles if needed, or rely on external CSS) ... */
        
        /* --- NEW: CUSTOM GRID LAYOUT SYSTEM --- */
        .custom-layout-grid {
            display: grid;
            grid-template-columns: 2fr 1fr; /* Default: Left (PFD) is wider */
            grid-template-areas: 
                "pfd    side"
                "nav    side"
                "vsd    side";
        ...
        #layout-area-pfd     { grid-area: pfd; }
        #layout-area-side    { grid-area: side; display: flex; flex-direction: column; gap: 12px; }
        #layout-area-nav     { grid-area: nav; }
        #layout-area-vsd     { grid-area: vsd; }
    ...
    <div class="info-window-content">
        ...
        <div class="unified-display-main-content">
            <div id="ac-tab-flight-data" class="ac-tab-pane active">
                ...
                <div class="custom-layout-grid" id="user-custom-layout">
                    
                    <div id="layout-area-pfd">
                            ...
                            <div class="tech-content">
                                ...
                                <div class="tech-grid">
                                    <div class="tech-stat-card">
                                        <div class="tech-stat-header"><span class="tech-stat-label">Registration</span><i class="fa-solid fa-hashtag" style="font-size: 12px; color: #475569;"></i></div>
                                        <span class="tech-stat-value">${reg}</span>
                                    </div>
                                    <div class="tech-stat-card">
                                        <div class="tech-stat-header"><span class="tech-stat-label">Callsign</span><i class="fa-solid fa-tag" style="font-size: 12px; color: #475569;"></i></div>
    ...
    // NEW: Apply saved layout
    const savedLayout = localStorage.getItem('userLayoutMode') || 'default';
    if (typeof applyCustomLayout === 'function') {
        applyCustomLayout(savedLayout);
    }
}
...
function setupFilterSettingsWindowEvents() {
    ...
    const setUIFromState = () => {
        // Toggles
        const toggleAtc = document.getElementById('filter-toggle-atc');
        const toggleSat = document.getElementById('filter-toggle-satellite-mode');
        const toggleLabels = document.getElementById('filter-toggle-aircraft-labels');
        
        if (toggleAtc) toggleAtc.checked = mapFilters.hideAtcMarkers;
        if (toggleSat) toggleSat.checked = (currentMapStyle === MAP_STYLE_SATELLITE);
        if (toggleLabels) toggleLabels.checked = mapFilters.showAircraftLabels;
        ...
        // NEW: Radios - Window Layout
        const savedLayout = localStorage.getItem('userLayoutMode') || 'default';
        const layoutRadio = document.querySelector(`input[name="window-layout-mode"][value="${savedLayout}"]`);
        ...
        if (startPicker) startPicker.value = mapFilters.themeStartColor || '#121426';
        if (endPicker) endPicker.value = mapFilters.themeEndColor || '#121426';
    ...
    filterSettingsWindow.addEventListener('change', (e) => {
        ...
        // NEW: Handle Window Layout Logic
        if (target.name === 'window-layout-mode') {
            if (typeof applyCustomLayout === 'function') {
                applyCustomLayout(target.value);
            }
            return;
        }
