...
function populateAircraftInfoWindow(baseProps, plan, sortedRoutePoints, communityAircraftData) {
    ...
    if (hasPlan && simbriefAircraftValue) {
        planButtonHtml = `
            <button id="plan-this-flight-btn" class="pilot-stats-toggle-btn" data-departure="${departureIcao}" data-arrival="${arrivalIcao}" data-aircraft="${simbriefAircraftValue}" style="width: 100%; margin-top: 16px;">
    ...
    let psTitle = "ACTIVE"; let psIcon = "fa-user-check"; let psColor = "#4ade80"; let psDesc = "Pilot is active";
    ...
    windowEl.innerHTML = `
        ...
        <div class="unified-route-tabs-container" style="
            background: rgba(30, 41, 59, 0.75);
            backdrop-filter: blur(16px);
            margin: -32px 16px 16px 16px; /* -32px pulls it up over header */
            border-radius: 12px;
            border: 1px solid rgba(255,255,255,0.1);
            display: flex;
            flex-direction: column;
            ...
            <div class="ac-route-info-bar" style="
                ...
                <div class="route-visual" style="flex: 1; max-width: 220px; display: flex; flex-direction: column; align-items: center;">
                    <div class="flight-progress-track" style="height: 4px; background: rgba(255,255,255,0.1); border-radius: 4px; position: relative; width: 100%;">
                    ...
                    <div style="display: flex; justify-content: space-between; width: 100%; margin-top: 6px; font-size: 9px; color: #cbd5e1; font-weight: 700;">
                        ...
                        <span>ETE: <span id="ac-ete" style="color: #38bdf8;">${ete}</span></span>
                    </div>
                </div>
            ...
            <div class="ac-info-window-tabs" style="
                padding: 10px 16px;
                display: flex;
                align-items: center;
                justify-content: space-between;
                gap: 16px;">
                <div class="modern-view-switcher" id="main-data-switcher" style="flex: 1; background: rgba(10, 15, 25, 0.4); border-radius: 8px; padding: 4px; display: flex; position: relative; border: 1px solid rgba(255,255,255,0.05); height: 40px; gap: 4px;">
                    <button class="ac-info-tab-btn ${flightDataActiveClass}" data-tab="ac-tab-flight-data" style="flex: 1; border: none; background: ${currentActiveTab === 'ac-tab-flight-data' ? 'rgba(255,255,255,0.08)' : 'transparent'}; color: ${currentActiveTab === 'ac-tab-flight-data' ? '#fff' : '#94a3b8'}; font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; padding: 0 10px; border-radius: 6px; cursor: pointer; z-index: 1; transition: all 0.3s ease; display: flex; align-items: center; justify-content: center; gap: 8px;">
                        <i class="fa-solid fa-gauge-high" style="color: ${currentActiveTab === 'ac-tab-flight-data' ? '#38bdf8' : 'inherit'};"></i> Flight Display
                    </button>
                    <button class="ac-info-tab-btn pilot-tab-btn ${pilotReportActiveClass}" data-tab="ac-tab-pilot-report" data-user-id="${baseProps.userId}" data-username="${pilotUsername}" style="flex: 1; border: none; background: ${currentActiveTab === 'ac-tab-pilot-report' ? 'rgba(255,255,255,0.08)' : 'transparent'}; color: ${currentActiveTab === 'ac-tab-pilot-report' ? '#fff' : '#94a3b8'}; font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; padding: 0 10px; border-radius: 6px; cursor: pointer; z-index: 1; transition: all 0.3s ease; display: flex; align-items: center; justify-content: center; gap: 8px;">
                        <i class="fa-solid fa-id-badge" style="color: ${currentActiveTab === 'ac-tab-pilot-report' ? '#fbbf24' : 'inherit'};"></i> ${pilotReportTabText}
                    </button>
                </div>
            </div>
        </div>
        ...
        <div class="unified-display-main-content">
            ...
            <div class="ac-tab-pane ${flightDataActiveClass}" id="ac-tab-flight-data">
                    ...
                    <div class="info-right-col">
                        <div id="location-data-panel">
                            <div class="nav-header">
                                <span class="nav-title">LOCATION & ENV</span>
                                <div class="nav-status-indicator"><div class="nav-blink"></div> LIVE</div>
                            </div>
                            <div class="nav-grid-container">
                                <div class="nav-cell nav-span-2">
                                    <span class="nav-label"><i class="fa-solid fa-location-crosshairs"></i> LAT</span>
                                    <div class="nav-row"><span class="nav-value" id="ac-lat">${baseProps.position.lat.toFixed(3)}</span><span class="nav-unit">DEG</span></div>
                                </div>
                                <div class="nav-cell nav-span-2">
                                    <span class="nav-label"><i class="fa-solid fa-location-crosshairs"></i> LON</span>
                                    <div class="nav-row"><span class="nav-value" id="ac-lon">${baseProps.position.lon.toFixed(3)}</span><span class="nav-unit">DEG</span></div>
                                </div>
                                <div class="nav-cell nav-span-4" style="background: rgba(0,0,0,0.2); border-color: rgba(255,255,255,0.1);">
                                    <span class="nav-label" style="color: #e4e4e7;"><i class="fa-solid fa-map-pin"></i> LOCATION</span>
                                    <span class="nav-value" id="ac-location" style="font-size: 0.85rem; color: #38bdf8;">Locating...</span>
                                </div>
                                <div class="nav-cell nav-span-4" style="background: rgba(0,0,0,0.2); border-color: rgba(255,255,255,0.1);">
                                    <span class="nav-label" style="color: #e4e4e7;"><i class="fa-solid fa-tower-observation"></i> NEAREST AIRPORT</span>
                                    <div style="display: flex; justify-content: space-between; align-items: baseline;">
                                        <span class="nav-value large highlight" id="ac-nearest-apt">---</span>
                                        <span class="nav-value small" id="ac-nearest-apt-dist">---</span>
                                    </div>
                                </div>
                                <div class="nav-cell nav-span-2">
                                    <span class="nav-label"><i class="fa-solid fa-wind"></i> WIND</span>
                                    <span class="nav-value small accent" id="ac-env-wind">--- / --</span>
                                </div>
                                <div class="nav-cell nav-span-2">
                                    <span class="nav-label"><i class="fa-solid fa-temperature-half"></i> OAT</span>
                                    <span class="nav-value small accent" id="ac-env-oat">--°C</span>
                                </div>
                            </div>
                        </div>
                        
                        <div class="rules-module-container" style="margin-top: auto;">
                            <div class="rules-header">
                                <span>FLIGHT RULES</span>
                            </div>
                            <div class="rules-body">
                                <div id="flight-rules-display" class="flight-rules-badge badge-vfr">
                                    <i class="fa-solid fa-spinner fa-spin"></i> CALC...
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="tech-card ac-profile-card-new">
                    <div class="tech-card-header" style="background: var(--bg-panel); border-bottom: 1px solid var(--border-glass);">
                        <div style="display: flex; gap: 8px;">
                            <button class="ac-info-tab-btn display-toggle-btn profile-toggle-btn active" data-target="nd-view" style="padding: 4px 10px; font-size: 0.75rem; border: 1px solid var(--border-glass); border-radius: 4px; background: rgba(56, 189, 248, 0.1); color: #fff;">NAV DISPLAY</button>
                            <button class="ac-info-tab-btn display-toggle-btn profile-toggle-btn" data-target="fmc-view" style="padding: 4px 10px; font-size: 0.75rem; border: 1px solid var(--border-glass); border-radius: 4px; background: transparent; color: #94a3b8;">LEGS / FMC</button>
                        </div>
                    </div>
                    
                    <div id="nd-view" class="ac-tab-pane active" style="padding: 0;">
                        <div class="display-bezel" style="border-radius: 0; border: none; padding: 0;">
                            <div class="crt-container scanlines" style="border-radius: 0; border: none;">
                                <div id="nd-container">
                                    <iframe id="nav-display-frame" src="nd.html"></iframe>
                                </div>
                            </div>
                        </div>
                    </div>
                    
                    <div id="fmc-view" class="ac-tab-pane" style="padding: 0; display: none;">
                        <div class="fms-module-container" style="border: none; border-radius: 0;">
                            <div class="fms-columns">
                                <span class="col-wpt">WAYPOINT</span>
                                <span class="col-data text-center">CRS</span>
                                <span class="col-data text-right">DIST</span>
                            </div>
                            <div id="fms-legs-list" class="fms-list-scrollarea custom-scroll">
                                ${fmsLegsHtml}
                            </div>
                            <div class="fms-footer">
                                <div class="fms-stat">
                                    <span class="stat-label">DTG</span>
                                    <span class="stat-value" id="fms-total-dist">---- NM</span>
                                </div>
                                <div class="fms-stat">
                                    <span class="stat-label">ETE</span>
                                    <span class="stat-value" id="fms-total-ete">--:--</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="seat-sensor-wrapper">
                    <div class="sensor-header">
                        <span>COCKPIT SENSOR</span>
                        <span id="sensor-timer" style="font-family: 'JetBrains Mono', monospace; font-size: 0.7rem; color: #38bdf8;">00:00:00</span>
                    </div>
                    <div class="sensor-body">
                        <div class="cockpit-view">
                            <div id="seat-cpt" class="seat" data-role="CPT"></div>
                            <div id="seat-fo" class="seat" data-role="F/O"></div>
                            <i id="icon-parking-overlay" class="fa-solid fa-p cockpit-overlay-icon icon-parking"></i>
                            <i id="icon-coffee-overlay" class="fa-solid fa-mug-hot cockpit-overlay-icon icon-coffee"></i>
                            <i id="icon-cloud-overlay" class="fa-solid fa-cloud-arrow-up cockpit-overlay-icon icon-cloud"></i>
                        </div>
                        <div class="seat-status-display">
                            <span id="status-cpt-text" class="status-pill">---</span>
                            <span id="status-fo-text" class="status-pill">---</span>
                        </div>
                        <div id="seat-narrative-text">Initializing sensor link...</div>
                    </div>
                </div>
            ...
            <div class="ac-tab-pane ${pilotReportActiveClass}" id="ac-tab-pilot-report" style="display: ${pilotReportDisplay}; padding: 0;">
    ...
    // --- Next WP and ETE Interval (The Logic Timer) ---
    const updateSensorTimers = () => {
        let bestWpIndex = -1;
        let minScore = Infinity;
        const currentPos = currentAircraftPositionForGeocode || baseProps.position;
        const currentTrack = currentPos.heading_deg || 0;

        if (originalFlatWaypointObjects.length > 0) {
            originalFlatWaypointObjects.forEach((wp, idx) => {
                if (!wp.location) return;
                const d = getDistanceKm(currentPos.lat, currentPos.lon, wp.location.latitude, wp.location.longitude);
                const bearingTo = getBearing(currentPos.lat, currentPos.lon, wp.location.latitude, wp.location.longitude);
                const bearingDiff = Math.abs(normalizeBearingDiff(currentTrack - bearingTo));
        ...
        if (hasPlan) {
            const destWp = originalFlatWaypointObjects[originalFlatWaypointObjects.length - 1];
            if (destWp && destWp.location) {
                distanceToDestNM = getDistanceKm(currentPos.lat, currentPos.lon, destWp.location.latitude, destWp.location.longitude) / 1.852;
            }
        }
        ...
        updateAll('#ac-next-wp-dist', `${nextWpDistDisplay}<span style="font-size: 0.6rem; color: #64748b; margin-left: 2px;">NM</span>`, true);

        const elElapsed = document.getElementById('ac-elapsed');
        const elTotal = document.getElementById('ac-total');
        const elEte = document.getElementById('ac-ete');
        const sourceEte = document.getElementById('fms-total-ete');

        if (atdTimestamp) {
            const diffMs = Date.now() - new Date(atdTimestamp).getTime();
            if (diffMs > 0) {
                const h = Math.floor(diffMs / 3600000);
                const m = Math.floor((diffMs % 3600000) / 60000);
                if (elElapsed) elElapsed.textContent = `${String(h).padStart(2, '0')}:${String(m).padStart(2, '0')}`;
            }
        }
    ...
    mainTabBtns.forEach((btn, index) => {
        btn.addEventListener('click', (e) => {
            ...
            targetBtn.style.background = 'rgba(255,255,255,0.08)';

            const i = targetBtn.querySelector('i');
            if (i) {
                if (targetBtn.classList.contains('pilot-tab-btn')) {
            ...
            if (targetBtn.classList.contains('pilot-tab-btn')) {
                const uid = targetBtn.dataset.userId;
    ...
    // --- Dynamic Data Updating Hooks ---
    const phaseClass = flightPhase.toLowerCase().replace(' ', '-');
    let phaseIcon = 'fa-plane';
    if (flightPhase === 'CRUISE') phaseIcon = 'fa-plane';
    else if (flightPhase === 'CLIMB') phaseIcon = 'fa-plane-departure';
    else if (flightPhase === 'DESCENT') phaseIcon = 'fa-plane-arrival';
    else if (flightPhase === 'APPROACH') phaseIcon = 'fa-plane-arrival';
    ...
    if (baseProps.position.gs_kt > 50 && totalDistanceNM > 0) {
        const distanceToDestNM = getDistanceKm(baseProps.position.lat, baseProps.position.lon, originalFlatWaypointObjects[originalFlatWaypointObjects.length - 1]?.location?.latitude, originalFlatWaypointObjects[originalFlatWaypointObjects.length - 1]?.location?.longitude) / 1.852;
